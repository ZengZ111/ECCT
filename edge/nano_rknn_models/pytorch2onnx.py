...
def main():
    ...
    # 在导出 backbone_x（255）之后，添加以下代码：
    backbone_z = torch.randn([1, 3, 127, 127], device=device)
    export_onnx_file_path_z = './models/onnx/nanotrack_backbone_127.onnx'
    torch.onnx.export(backbone_net, backbone_z, export_onnx_file_path_z,
                    input_names=['input'], output_names=['output'],
                    verbose=True, opset_version=14)

    # 同时将原来的 255 导出改名为 nanotrack_backbone_255.onnx（可选）
    export_onnx_file_path= './models/onnx/nanotrack_backbone_255.onnx'
